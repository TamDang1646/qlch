...
export default class InputBase extends Component<InputBaseProps, InputBaseState> {
  ...
  _setText(text: string, callback?: () => void) {
    ...
    this.setState({
      value: text,
      text: this.props.type == 'MONEY' ? getMoneyFormat(text, this.props.currency == 'VND' ? '100,000.00' : '100.000,00') : text,
      focus: text.length > 0,
    }, () => {
      if (callback) {
        callback();
  ...
  _onChangeText = async (text: string) => {
    ...
    if (this.props.type == 'MONEY') {
      ...
      text = getMoneyFormat(text, this.props.currency == 'VND' ? '100,000.00' : '100.000,00');
    }
